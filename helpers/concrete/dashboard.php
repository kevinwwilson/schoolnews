...
class ConcreteDashboardHelper {
	...
	public function getDashboardPaneHeaderWrapper1($title = false, $help = false, $span = 'span12', $includeDefaultBody = true, $navigatePages = array(), $upToPage = false, $favorites = true) {
		...
		$html = '<div class="ccm-ui"><div class="row"><div class="' . $span . '"><div class="ccm-pane">';
		$html .= self::getDashboardPaneHeader1($title, $help, $navigatePages, $upToPage, $favorites);
		if ($includeDefaultBody) {
			$html .= '<div class="ccm-pane-body ccm-pane-body-footer">';
		}
		return $html;
	...
	public function getDashboardPaneHeader1($title = false, $help = false, $navigatePages = array(), $upToPage = false, $favorites = true) {
		$c = Page::getCurrentPage();
		$vt = Loader::helper('validation/token');
		$token = $vt->generate('access_quick_nav');
		...
		if (count($trail) > 1 || count($navigatePages) > 1 || is_object($upToPage)) { 
			...
			if (count($subpagesP) > 0 || is_object($upToPage)) { 
				...
				$relatedPages .= '<li class="nav-selected"><a href="'.BASE_URL.DIR_REL.'/index.php/dashboard/pronews/shedule_news_group/">Group List</a></li>';
