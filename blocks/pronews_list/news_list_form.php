...
<div id="ccm-pagelistPane-add" class="ccm-pagelistPane">
	...
	<div class="news-attributes">
		<div>
        <strong><?php  echo t('Category')?></strong><br/>
        ...
        <div>
        <strong><?php  echo t('Tag')?></strong><br/>
			<select name="tagss">
			<?php  
			$db = Loader::db();
			$k=$db->query("SELECT akID FROM AttributeKeys WHERE akHandle = 'news_tag'");
			while($row=$k->fetchrow()){
				$key=$row['akID'];
			}
			$r=$db->query("SELECT value FROM atSelectOptions WHERE akID = $key");
			...
			if(is_array($tags)){
				foreach($tags as $tag){
					if ($tag==$tagss){ $select = 'selected';}else{ $select = '';}
					echo '<option value="'.$tag.'" '.$select.'>'.$tag.'</option>';
				}
				if (!isset($tagss) || $tagss=='All Tags'){ 
					echo '<option value="All Tags" selected>All Tags</option>';
				}else{
					echo '<option value="All Tags">All Tags</option>';
				}
			}
        ...
        <div>
        <strong><?php  echo t('District')?></strong><br/>
			<select name="distss">
			<?php  
			$db = Loader::db();
			$k=$db->query("SELECT akID FROM AttributeKeys WHERE akHandle = 'district'");
			while($row=$k->fetchrow()){
				$key=$row['akID'];
			}
			$r=$db->query("SELECT value FROM atSelectOptions WHERE akID = $key");
			...
			if(is_array($dists)){
				foreach($dists as $dis){
					if ($dis==$distss){ $select = 'selected';}else{ $select = '';}
					echo '<option value="'.$dis.'" '.$select.'>'.$dis.'</option>';
				}
				if (!isset($distss) || $distss == 'All District'){ 
					echo '<option value="All District" selected>All District</option>';
				}else{
					echo '<option value="All District">All District</option>';
				}
			}
